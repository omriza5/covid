import { getCovidData } from "./covidService.js";
import { getAllCountries, mapCountriesToRegions } from "./countriesService.js";
import { mapCovidDataToRegions } from "./covidService.js";
...
export const prepareAllData = async () => {
  try {
    // replace countriesDb with a real call to server
    // const countries = await getAllCountries();
    const mapped = mapCountriesToRegions(countriesDb);
    const covidData = await getCovidData();

    const merged = mergeData(mapped, covidData);
    const final = mapCovidDataToRegions(merged);
    localStorage.setItem("covidData", JSON.stringify(final));
    return final;
  } catch (error) {
    console.log("PREPARE ALL DATA: ", error);
  }
};

const mergeData = (regions, covidData) => {
  const mappedRegions = [
    ...regions[0].Asia,
    ...regions[1].Europe,
    ...regions[2].Africa,
  ...
  const result = mappedRegions.map((country) => {
    const found = covidData.find((c) => c.code === country.code);
    if (found) {
      return {
        ...country,
        latestData: found.latest_data,
        today: found.today,
      };
    }
  });
